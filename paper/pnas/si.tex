...
%\section{Pressure Broadening}
%In the main text we neglect pressure broadening and temperature scaling when discussing the \Ts-invariance of $\tauk(T)$. Here we justify these approximations.
%
%Temperature scaling of absorption coefficients is consistent with $\tauk$ being a function of temperature only, and does not introduce any new \Ts-dependence of $\tauk(T)$. This justifies neglecting it in Eqn. (3) in the main text.  Pressure broadening, on the other hand, cannot be neglected with such a straightforward argument. Rather, we compute profiles of $\tauk(T)$ for our RCE simulations both with and without pressure broadening and then compare them (Fig. \ref{tau_profiles}).
%
% The left panel of Fig. \ref{tau_profiles}shows $\tauk(T)$ as computed via Eqn. (3) with $\kappa(\lambda) = 1 \ \meter^2/\kg$ (this value is arbitrary and immaterial, since any variation in $\kappa$ simply produces a horizontal translation in such a log plot) and the upper limit of integration taken at model top rather than infinity. The right panel shows a pressure-broadened version of this quantity which has a factor of $p/\ps$ in the integral, but is otherwise identical. One can see that while pressure broadening does change the absolute values of the $\tauk(T)$ profiles (note the difference in, say, \tauk(180\ \Kelvin)$ between the panels), and also introduces additional spread between the $\tauk(T)$ profiles at differing \Ts, this spread is minimal. If we focus on $\tau =1$ where emission-to-space is concentrated, the spread between profiles is roughly 5 K. Assuming linearity of the planck function $B(\lambda,T)$ in $T$ at fixed \lambda, this translates to a roughly 5 K/250 K = 2\% variation in $B(\lambda,T)$ with \Ts\ when evaluated at fixed $\kappa(\lambda) $ and $\tauk$
% 
%does introduce some \Ts-dependence to $\tauk(T)$, as shown in Fig. \ref{tau_profiles}.
%%Figure tau_profiles
%\begin{figure}[h]
%        \begin{center}
%                        \includegraphics[scale=0.7]{\figurepath tau_profiles.pdf}
